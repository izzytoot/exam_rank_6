...
int main(int ac, char **av) {
	...
	while (1) {
		...
		for (int fd = 0; fd <= max_fd; fd++){
			...
			if (fd == sockfd) {
				int connfd = accept(sockfd, (struct sockaddr *)&servaddr, &addr_len);
				...
				memset(clients[connfd].msg, 0, sizeof(clients[connfd].msg));
				FD_SET(connfd, &activefds);
				sprintf(buffer_write, "server: client %d just arrived\n", clients[connfd].id);
				broadcast(connfd, buffer_write);
			} 
			
			else {
				int n = recv(fd, buffer_read, 100000, 0);
				if (n <= 0) {
					sprintf(buffer_write, "server: client %d just left\n", clients[fd].id);
					broadcast(fd, buffer_write);
					FD_CLR(fd, &activefds);
					close(fd);
				}
